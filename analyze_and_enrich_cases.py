...
def process_all_cases():
    ...
    print(f"Found {cases.count()} active cases to review.")
    
    count = 0
    for case in cases:
        # Check if it already has the enriched structure
        needs_analysis = "【客户案例分析】" not in case.insurance_needs
        needs_planning = "【财富管理规划】" not in case.case_description
        
        if needs_analysis or needs_planning:
            ...
            if result:
                if needs_analysis:
                    ...
                    case.insurance_needs = f"{result['analysis']}\n\n原始需求：{case.insurance_needs}"
                
                if needs_planning:
                    # Prepend planning to existing description
                    case.case_description = f"{result['planning']}\n\n原始方案：{case.case_description}"
            ...
            print(f"Skipping case {case.id}: Already enriched.")
            
    print(f"Finished. Enriched {count} cases.")
