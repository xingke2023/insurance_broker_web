...
def call_gemini_with_fallback(model, contents, config=None, operation_name="API调用", timeout=180):
    """
    ...
    Args:
        ...
        timeout: API调用超时时间（秒），默认180秒（3分钟）
    ...
    # 定义API调用函数
    def _call_api(client, model, contents, config):
        if config:
    ...
    for key_name, api_key in api_keys:
        try:
            logger.info(f"🔑 [{operation_name}] 尝试使用{key_name}: {api_key[:10]}... (超时: {timeout}秒)")

            client = genai.Client(api_key=api_key)

            # 使用线程池执行API调用，设置超时
            with ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(_call_api, client, model, contents, config)
                try:
                    response = future.result(timeout=timeout)
                    logger.info(f"✅ [{operation_name}] {key_name}调用成功")
                    return response
                except FutureTimeoutError:
                    logger.error(f"⏰ [{operation_name}] {key_name}超时（{timeout}秒）")
...
🚨 数据完整性要求（重要）：
必须提取表格的所有行，从第1年到表格的最后一行，不要遗漏跨页的数据行。
根据计划书的保险期限（如至100岁、至138岁、终身等）提取对应年份的完整数据。
保单年度终结必须连续（1, 2, 3, 4...），不要跳过中间年份。
表格通常会跨越多个页面，必须逐页提取，直到表格真正结束。
...
重要提醒：
1. 你的返回必须包含这4个顶层字段，缺一不可！
2. policy_values_table必须提取完整数据，行数应该与保险期限匹配（如至138岁应提取138行）
3. 确保提取的数据连续且准确，从第1年到最后一年，不要遗漏任何行！
"""
        ...
        # 使用支持fallback的API调用（3分钟超时）
        logger.info(f"⏰ 开始调用Gemini API（超时设置: 180秒）")
        response = call_gemini_with_fallback(
            model='gemini-3-flash-preview',
            contents=contents,
            config=config,
