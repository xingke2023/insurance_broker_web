...
const AIHub = () => {
  ...
  const categories = [
    {
      ...
      description: '集成计划书自动生成、保司数据提取、公众号智能客服及保司通知推送，让团队告别低效手动操作。',
      icon: Briefcase,
      path: '/ai-office',
      color: 'blue',
      ...
      features: ['计划书自动生成系统', '公众号智能客服系统', '保司通知推送系统', '计划书数据提取']
    },
    {
      title: 'AI赋能营销：自媒体运营',
      ...
      features: ['热点敏感度推送', '保险业内容制作', '全平台自动发布', 'OpenClaw 控制终端']
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-blue-500/30">
      ...
      {/* Main Categories - Balanced Two Blocks */}
      <div className="max-w-7xl mx-auto px-6 pb-32 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          {categories.map((item, index) => (
            <div 
              key={index}
              onClick={() => onNavigate(item.path.substring(1))}
                ...
                <div className="grid grid-cols-2 gap-4 mb-12">
                  {item.features.map((feature, fIdx) => (
                    <div key={fIdx} className="flex items-center space-x-2">
                       <div className={`w-1.5 h-1.5 rounded-full ${item.bgClass.replace('/10', '')}`} />
