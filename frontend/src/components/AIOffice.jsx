...
const AIOffice = () => {
  ...
  const coreTools = [
    { title: '计划书自动分析', icon: FileText, path: '/plan-analyzer' },
    { title: '数据自动提取', icon: Database, path: '/plan-management' },
    { title: '计划书擦除工具', icon: Eraser, path: '/pdf-footer-remover2' },
    { title: '保司通知推送', icon: Bell, path: '/dashboard' },
    { title: '全港产品查询', icon: Search, path: '/insurance-products' },
  ];

  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      {/* Simple top nav */}
      <div className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white/90 backdrop-blur-xl">
        <button
        ...
        <div className="text-sm font-bold tracking-widest uppercase text-slate-400 font-mono">AI Office</div>
        <button
          ...
          <div className="p-8 md:p-12 bg-slate-900 text-white relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
              ...
              <button
                ...
                className="px-8 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 transition-colors font-bold whitespace-nowrap"
              >
                立即使用 AI 顾问
              </button>
            </div>
      ...
      <section ref={planSystemRef} className="max-w-7xl mx-auto px-6 py-24 scroll-mt-24">
        <div className="flex flex-col lg:flex-row items-center gap-16">
          <div className="lg:w-1/2">
            ...
            <h2 className="text-4xl font-black mb-8 tracking-tight">港险团队计划书分析系统</h2>
            <div className="space-y-8">
              {[
                { title: '上传 PDF 计划书', icon: FileText, desc: '支持所有主流保险公司的计划书格式，一键上传即可开始分析。' },
                { title: 'AI 自动提取数据', icon: Monitor, desc: '系统自动识别受保人信息、缴费年期、年度价值表等核心数据。' },
                { title: '多保司智能对比', icon: Database, desc: '将多份计划书数据汇总对比，直观呈现各产品的收益差异。' }
              ].map((item, i) => (
            ...
            <button
              onClick={() => onNavigate('plan-analyzer')}
          ...
          <div className="lg:w-1/2 relative">
            ...
            <div className="relative p-8 bg-white rounded-3xl shadow-2xl border border-slate-100">
              <div className="space-y-4">
                {['计划书智能分析', '产品对比工具', 'AI 财务顾问', 'PDF 工具箱 Pro'].map((item, i) => (
      ...
      <section className="max-w-7xl mx-auto px-6 py-24 border-t border-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            { title: '产品查询系统', icon: Search, desc: '全港主流保险产品库极速检索。', path: '/insurance-products' },
            { title: 'AI 财务顾问', icon: Bell, desc: '基于真实案例库的智能问答顾问。', path: '/customer-cases' }
          ].map((tool, i) => (
            <div
              key={i}
              onClick={() => onNavigate(tool.path.substring(1))}
      ...
      <div className="bg-slate-900 py-24 text-center px-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <button
            onClick={() => onNavigate('dashboard')}
            className="px-10 py-4 rounded-2xl bg-blue-600 text-white font-bold hover:bg-blue-500 transition-all shadow-xl shadow-blue-500/20"
          >
            进入控制台
          </button>
          <button
            onClick={() => onNavigate('ai-hub')}
            className="px-10 py-4 rounded-2xl bg-white/5 text-white border border-white/10 font-bold hover:bg-white/10 transition-all"
          >
            返回 AI Hub
          </button>
        </div>
